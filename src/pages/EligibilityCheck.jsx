import { ArrowRight, FileUp, GraduationCap, ShieldCheck } from 'lucide-react'
...
function EligibilityCheck() {
  ...
  const submitForm = async (event) => {
    ...
    try {
      ...
      setErrorMessage('We could not submit your profile. Please try again or contact us on +94 70 767 0670.')
      setStatus('error')
    }
  }
        ...
        <div className="container eligibility-layout">
          ...
          <form className="eligibility-form" onSubmit={submitForm}>
            {status === 'success' && assessment && (
              <section className={`assessment-result assessment-${assessment.tone}`} role="status">
                <div className="assessment-heading">
                  ...
                  <div className="assessment-score"><strong>{assessment.total}</strong><span>/ 100</span></div>
                </div>
                <div className="assessment-bars">
                  {assessment.categories.map((category) => <div key={category.label}><span>{category.label}<b>{category.score}/{category.max}</b></span><progress value={category.score} max={category.max} /></div>)}
                </div>
            ...
            <section className="eligibility-section">
              ...
              <div className="eligibility-grid">
                ...
                <Field label="Mobile / WhatsApp Number" name="phone" type="tel" required maxLength="30" />
            ...
            <section className="eligibility-section">
              ...
              <RadioGroup legend="Academic relevance to your preferred course" name="academic_relevance" required options={['Unrelated field', 'Some related modules', 'Related degree', 'Directly related degree + specialization']} />
              <RadioGroup legend="Is a study gap of more than 5 years clearly explained by work or other evidence?" name="gap_explained" options={['Yes', 'No', 'Not Applicable']} />
            </section>
            ...
            <section className="eligibility-section">
              ...
              <RadioGroup legend="Preferred Study Level" name="preferred_level" required options={['Foundation', 'Diploma', 'Bachelor’s Degree', 'Top-Up Degree', 'Postgraduate Diploma', 'Master’s Degree', 'PhD / Research', 'Not Sure – Need Guidance']} />
              <RadioGroup legend="Preferred Intake" name="preferred_intake" required options={['January / February', 'May / June', 'September / October', 'Other']} />
              <Field label="Preferred Year of Study" name="preferred_year" type="number" min="2026" max="2040" required />
            </section>

            <section className="eligibility-section">
              <header><span>05</span><div><h2>Work Experience</h2><p>Add any relevant professional experience.</p></div></header>
              <RadioGroup legend="Do you have work experience?" name="has_work_experience" required options={['Yes', 'No']} />
              <div className="eligibility-grid"><Field label="Current / Most Recent Job Title" name="job_title" maxLength="100" /><Field label="Company" name="company" maxLength="120" /><Field label="Total Years of Work Experience" name="work_experience_years" type="number" min="0" max="70" step="0.5" /></div>
            </section>
            ...
            <section className="eligibility-section">
              ...
              <RadioGroup legend="Strength of funding source" name="funding_strength" required options={['Third-party sponsor', 'Parent / spouse with explainable income']} />
              <RadioGroup legend="Are funds currently available for your studies?" name="funds_available" required options={['Yes', 'Partially Available', 'Need Financial Guidance']} />
            </section>
